...
import { Leyenda, Leyendas, TooltipCaja, TooltipFila } from "@/components/dashboard/charts";
import { AMBAR, AXIS_TICK, HOY, REJILLA, TINTA_MUTED, fmtH, fmtPct1 } from "@/components/dashboard/charts-tema";
 ...
 *
 * Este archivo trae recharts; `cobertura-semana.tsx` lo carga con
 * `next/dynamic` (`ssr: false`), pinta el `<figure>` exterior y resuelve el
 * caso sin datos. Aquí se asume `cobertura.length > 0`.
 */
...
export type CoberturaSemanaProps = {
...
/** Gráfica y leyenda (sin el `<figure>` exterior, que pone `cobertura-semana.tsx`). */
export default function CoberturaSemanaGrafica({
  cobertura,
  deficitPicoHoras = 0,
}: Omit<CoberturaSemanaProps, "className">) {
